...
**Implementation status:** Matches shipped MVP (TASKS 01–26); docs synced TASK 27
...
|----------------|---------|
| Splash Screen | Native Compose UI (CRT boot flicker, wordmark) |
...
- Motion: splash flicker/floor 400–600ms (default 500ms); cross-fades 200ms; Retry label swap 150ms
...
- **No** permission-prompt overrides (app does not request geolocation, camera, or microphone)
...
| Retry from Offline | Explicit user Retry → Retrying (RECONNECTING…) → Loading → Ready or Offline |
...
Connectivity: `ConnectivityMonitor` + `ConnectivityShellBridge` emit `NetworkLost` on cold offline and Available→Unavailable. Restoration does **not** auto-leave Offline — Retry enablement reads live connectivity.
...
| CI compile gate | Codemagic on Linux |
...
1. ~~Exact CRT accent hex~~ — shipped `#00FF41`; monospace is system Monospace until a licensed Courier New asset is bundled.
