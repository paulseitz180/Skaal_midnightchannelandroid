 ...
 *
 * Single exhaustive model for Splash → Loading → Ready → Offline → Retrying.
...
sealed interface ShellState {
    ...
    /**
     * Cold start / CRT boot.
     * Advances when **both** floor (~400–600ms) and first content readiness fire.
     */
    data class Splash(
...
/**
 * WebView content should be visible (not covered by Splash / Offline).
